...
def get_role(user):
    ...
    if user.is_authenticated:
        if user.is_superuser:
            template = "point_owner"
        elif user.groups.filter(name='point_owner').exists():
            template = "point_owner"
...
class Students(models.Model):
    model = models.CharField('ФИО', max_length=50, default="")
    color = models.CharField('Группа', max_length=50, default="")
...
class Facultets(models.Model):
    model_id = models.ForeignKey('facultet', on_delete=models.CASCADE, default=0)
